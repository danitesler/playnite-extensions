...
function Invoke-ThemeCompose {
    <#
        Builds a loadable Playnite theme directory:
          1. kit overlay   (<themeKit>/<Mode>/**) and the kit's LICENSE*.txt notices
          2. Constants.xaml rendered from <themeKit>/Constants.template.xaml + <palette>
    ...
    $copyTree = {
        ...
        foreach ($file in Get-ChildItem -Path $Source -Recurse -File) {
            ...
            if ((Test-Path $target) -and $Label -eq "theme") {
                Write-Host "  theme overrides $relative"
            }
    ...
    if ($Profile.themeKit) {
        $kitRoot = Join-RepoPath $Profile.themeKit
        $kitOverlay = Join-Path $kitRoot $mode
        if (-not (Test-Path $kitOverlay)) {
            throw "Theme kit has no '$mode' overlay at $kitOverlay"
        }
        & $copyTree $kitOverlay "kit"
        ...
        $template = Join-Path $kitRoot "Constants.template.xaml"
        if ($Profile.palette -and (Test-Path $template)) {
            ...
            $header = "<!-- Generated by scripts/build-theme.ps1 from $($Profile.themeKit)/Constants.template.xaml and $($Profile.palette). Edit those, not this file. -->`n"
