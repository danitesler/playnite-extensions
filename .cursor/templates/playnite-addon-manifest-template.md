...
Type: (Generic or MetadataProvider or GameLibrary)
...
2. `Type` uses the database names, not the `extension.yaml` ones: `Generic` for GenericPlugin, `MetadataProvider` for MetadataPlugin, `GameLibrary` for LibraryPlugin.
